...
diag_log format["WASTELAND SERVER - Special Mission Started: %1",_missionType];
...
diag_log format["WASTELAND SERVER - Special Mission Waiting to run: %1 in %2 min",_missionType, sideMissionDelayTime / 60];
[specialMissionDelayTime] call createWaitCondition;
diag_log format["WASTELAND SERVER - Special Mission Resumed: %1",_missionType];

[_missionMarkerName,_randomPos,_missionType] call createClientMarker;

_hint = parseText format ["<t align='center' color='%2' shadow='2' size='1.75'>Special Objective</t><br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%3' size='1.25'>%1</t><br/><t align='center' color='%3'>A guerrilla squad has been spotted near the marker</t>", _missionType,  sideMissionColor, subTextColor];
[nil,nil,rHINT,_hint] call RE;

CivGrpSp = createGroup civilian;
[CivGrpSp,_randomPos] spawn createRandomGroup;
_currentCalloutAlive = 100; // Big dummy number to show remaining guerrillas before anyone is killed

diag_log format["WASTELAND SERVER - Special Mission Waiting to be Finished: %1",_missionType];
#ifdef __A2NET__
_startTime = floor(netTime);
#else
_startTime = floor(time);
...
if(_result == 1) then
{
    ...
    }forEach units CivGrpSp;
    deleteGroup CivGrpSp;
	...
	[nil,nil,rHINT,_hint] call RE;
    diag_log format["WASTELAND SERVER - Special Mission Failed: %1",_missionType];
} else {
	//Mission Complete.
    deleteGroup CivGrpSp;
	...
	[nil,nil,rHINT,_hint] call RE;
    diag_log format["WASTELAND SERVER - Special Mission Success: %1",_missionType];
};
